...
class Google_Analytics_4_EEC_Pixel extends Google_Analytics
{
    use Trait_Google;
    ...
    public function inject_order_received_page($order, $order_total, $order_item_ids, $is_new_customer)
    {
        ...
        ?>
                ...
                if  ((typeof wooptpm !== "undefined") && !wooptpm.isOrderIdStored(<?php echo $order->get_id() ?>)) {
                    gtag('event', 'purchase', <?php echo $this->get_event_purchase_json($order, $order_total, $order_currency, $is_new_customer, 'ga_4') ?>);
                }
        <?php
    }
}